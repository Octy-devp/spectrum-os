...
| **v1.1** | 2026-09-16 | ⚠️ 見下方「v1.1 的 commit hash 悖論」 | **（乙）agy 獨立終審指出兩漏洞 ＋（丙）本律自身無修訂條款** | ① 第二條新增 **2.5 授權交接機制**（解 v1.0 與 ECC `AGENTS.md:235` 的 push 對撞＋新增 2.5.3 凍結期推送檢查）② 第七條**四層→五層**（新增 **7.4 tooling/governance**：五問決策樹＋本層三紀律）③ 新增**第九條 修訂程序**（四類動機／五格草案／生效條件／版本號與留痕）④ 附錄 A 登錄本輪審計列、附錄 B 補第 12–14 列、檔尾新增本節。**原八條編號與內容除第二／七條被修訂者外未動** |

#### ⚠️ v1.1 的 commit hash 悖論（新增的第二處「有紀律、無防線」）
...
**現行裁決（v1.1 採用 ② 的變體，並在此說明）**：**本檔的修訂記錄對「本次 commit 自身」採引用式記載**——hash 的事實來源是 `git log`，理由是：
